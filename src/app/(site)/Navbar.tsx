...
export default function Navbar({ session, signOut, showSearch }: any) {
  ...
  return (
    <nav className="fixed top-0 z-50 py-4 w-full bg-lightGray/10">
        ...
        <div className="items-center gap-2 md:flex">
          {session ? (
            <div className="flex items-center gap-5">
              ...
              <Button
                type="button"
                className="w-full bg-mintGreen hover:bg-[#3C3C3C] text-white/90 transition"
                onClick={signOut}
              >
                Sign Out
              </Button>
            ...
            <Link
              href={"/login"}
              className="flex items-center gap-2 rounded-md bg-mintGreen px-10 py-3 text-sm text-white font-bold transition hover:bg-[#3C3C3C]"
            >
              Login
            </Link>
          )}
